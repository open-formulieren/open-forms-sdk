        ...
        {!isLoading && (
          <FormioForm
            ...
            onChange={values => {
              ...
              // Only run frontend logic if the feature flag is enabled, or if any of the rules
              // require the backend.
              if (
                !form.newLogicEvaluationEnabled ||
                step?.formStep.logicRules.some(rule => rule.requireBackend)
              ) {
                scheduleLogicCheck();
              } else {
                runFrontendLogic();
