...
Note that the anonymous form start option is implemented as a button with `type="submit"` and is therefore
expected to be nested inside of a `<form>` element with an appropriate `onSubmit` handler.
