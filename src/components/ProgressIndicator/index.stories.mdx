...
The `ProgressIndicatorDisplay` component shows the different steps in a form. The style of each
step varies depending on whether it is applicable, it is completed or it is the currently active step.

export const DisplayTemplate = (args) => (
  <MemoryRouter>
    <ProgressIndicatorDisplay {...args} />
  </MemoryRouter>
);
...
* If the steps are currently active, completed, applicable or if a step can be clicked to navigate to it.
