...
export const DynamicTemplate = args => <AsyncSelectField {...args} />;
...
const test = async canvasElement => {
  ...
  await expect(canvas.getByText('Option')).toBeVisible();
  await expect(canvas.getByText('This is a custom description for the option field')).toBeVisible();
