...
export default class Children extends Field {
  ...
  setValue(value, flags = {}) {
    ...
    // Normalize each child to ensure `__selected` and `__addedManually` are always a
    // boolean (used to track the added/selected children by the user)
    value = value.map(child => ({
      ...child,
      __selected: typeof child.__selected === 'boolean' ? child.__selected : false,
  ...
  handleChildSave(newChild) {
    const childExists = this.dataValue.find(child => child.bsn === newChild.bsn);

    let updated;
    if (childExists) {
      // Update the existing child
      updated = this.dataValue.map(child =>
        child.bsn === newChild.bsn ? {...child, ...newChild, __addedManually: true} : child
      );
    } else {
  ...
  toggleChildSelection(childBSN) {
    this.dataValue = this.dataValue.map(child => {
      if (child.bsn === childBSN) {
        return {...child, __selected: !child.__selected};
      }
      return child;
    });
