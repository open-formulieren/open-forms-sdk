...
const maxNFilesForm = {
  type: 'form',
  components: [
    {label: 'Upload multiple files', key: 'multipleFiles', type: 'file', multiple: true, maxNumberOfFiles: 2},
  ],
...
describe('Multiple File Component', () => {
  ...
  test('Uploading 3 files gives an error', (done) => {
    ...
    Formio.createForm(element, formJSON).then(form => {
      ...
      expect(component.error.message).toEqual('Too many files added. The maximum allowed number of files is 2.');
      done();
